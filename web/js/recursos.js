...
$(document).on('click', '#btn-Editar', function () {
    ...
    var repuesta = confirm("¿Realmente desea editar los datos?");
    if (repuesta) {
        var fila = $(this).closest("tr");
        var ida = fila.find("td:eq(0)").text();
        $.post('NewServlet2?menu=players', {accion: "editar", id: ida},
                function (response) {
...
//alert(jsonString);
        ...
        var ide = fila.find("td:eq(0)").text();
        $.post('NewServlet2?menu=players', {accion: "editar", id: ide},
                function (response) {
                    cargarTabla();
                    alert(response);
                });
